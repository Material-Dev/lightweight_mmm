...
_MODEL_FUNCTION_NAMES = {
    "linear": models.media_mix_model,
    "multiplicative": models.multiplicative_media_mix_model,
}
...
def _compare_equality_for_lmmm(item_1: Any, item_2: Any) -> bool:
    ...
    if type(item_1) != type(item_2):
        ...
        isinstance(item_1, Sequence) and not all(isinstance(x, str) for x in item_1)
    ):
...
class LightweightMMM:
    ...
    def __post_init__(self):
        ...
        self._model_function = _MODEL_FUNCTION_NAMES[self.model_spec]
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        def _create_list_of_attributes_to_compare(mmm_instance: Any) -> Sequence[str]:
            ...
            attributes_which_have_been_instantiated = [
                x
                for x in all_attributes_that_can_be_compared
    ...
    def _preprocess_custom_priors(
        self, custom_priors: Dict[str, Prior]
    ) -> MutableMapping[str, Prior]:
        ...
        for prior_name in custom_priors:
            if isinstance(custom_priors[prior_name], numbers.Number):
                ...
                custom_priors[prior_name], collections.abc.Sequence
            ) and not isinstance(custom_priors[prior_name], str):
                custom_priors[prior_name] = default_priors[prior_name].__class__(
                    *custom_priors[prior_name]
                )
    ...
    def fit(
        ...
        init_strategy: Callable[
            [Mapping[Any, Any], Any], jnp.ndarray
        ...
        if media.shape[1] != len(media_prior):
            raise ValueError(
                "The number of data channels provided must match the "
        ...
        if custom_priors:
            ...
            custom_priors = self._preprocess_custom_priors(custom_priors=custom_priors)
        ...
        if weekday_seasonality and seasonality_frequency == 52:
            logging.warn(
                "You have chosen daily seasonality and frequency 52 "
    ...
    def _predict(
        ...
        return infer.Predictive(model=model, posterior_samples=posterior_samples)(
            rng_key=rng_key,
    ...
    def predict(
        ...
        if not hasattr(self, "trace"):
            raise NotFittedModelError(
                "Need to fit the model before running " "predictions."
            )
        if media_gap is not None:
            if media.ndim != media_gap.ndim:
                raise ValueError(
                    "Original media data and media gap must have the same "
                    "number of dimensions."
                )
            if media.ndim > 1 and media.shape[1] != media_gap.shape[1]:
                raise ValueError(
                    "Media gap must have the same numer of media channels"
            ...
            if extra_features is not None:
                previous_extra_features = jnp.concatenate(
                    arrays=[
                        ...
                        jnp.zeros(
                            (media_gap.shape[0], *self._extra_features.shape[1:])
                        ),
                    ],
                    axis=0,
                )
    ...
    def get_posterior_metrics(
        ...
        if not target_scaler:
            logging.warning(
                "Target scaler was not given and unscaling of the target "
        ...
        media_contribution = jnp.einsum(
            einsum_str,
            self.trace["media_transformed"],
