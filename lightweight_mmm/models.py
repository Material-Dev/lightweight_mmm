...
Prior = Union[dist.Distribution, Dict[str, float], Sequence[float], float]
...
TRANSFORM_PRIORS_NAMES = immutabledict.immutabledict(
    {
        "carryover": frozenset(
            (_AD_EFFECT_RETENTION_RATE, _PEAK_EFFECT_DELAY, _EXPONENT)
        ),
        "adstock": frozenset((_EXPONENT, _LAG_WEIGHT)),
        "hill_adstock": frozenset(
            (_LAG_WEIGHT, _HALF_MAX_EFFECTIVE_CONCENTRATION, _SLOPE)
        ),
    }
)
    ...
    return immutabledict.immutabledict(
        {
            ...
            _COEF_TREND: dist.Normal(loc=0.0, scale=1.0),
            _EXPO_TREND: dist.Uniform(low=0.5, high=1.5),
            _SIGMA: dist.Gamma(concentration=1.0, rate=1.0),
            _GAMMA_SEASONALITY: dist.Normal(loc=0.0, scale=1.0),
            _WEEKDAY: dist.Normal(loc=0.0, scale=0.5),
            _COEF_EXTRA_FEATURES: dist.Normal(loc=0.0, scale=1.0),
            _COEF_SEASONALITY: dist.HalfNormal(scale=0.5),
        }
    )
            ...
            "carryover": immutabledict.immutabledict(
                {
                    ...
                    _PEAK_EFFECT_DELAY: dist.HalfNormal(scale=2.0),
                    _EXPONENT: dist.Beta(concentration1=9.0, concentration0=1.0),
                }
            ),
            "adstock": immutabledict.immutabledict(
                {
                    _EXPONENT: dist.Beta(concentration1=9.0, concentration0=1.0),
                    _LAG_WEIGHT: dist.Beta(concentration1=2.0, concentration0=1.0),
                }
            ),
            "hill_adstock": immutabledict.immutabledict(
                {
                    _LAG_WEIGHT: dist.Beta(concentration1=2.0, concentration0=1.0),
                    _HALF_MAX_EFFECTIVE_CONCENTRATION: dist.Gamma(
                        concentration=1.0, rate=1.0
                    ),
                    _SLOPE: dist.Gamma(concentration=1.0, rate=1.0),
                }
            ),
        }
    )
...
def transform_hill_adstock(
    ...
    with numpyro.plate(
        name=f"{_HALF_MAX_EFFECTIVE_CONCENTRATION}_plate", size=media_data.shape[1]
    ):
    ...
    with numpyro.plate(name=f"{_SLOPE}_plate", size=media_data.shape[1]):
        slope = numpyro.sample(
            name=_SLOPE, fn=custom_priors.get(_SLOPE, transform_default_priors[_SLOPE])
        )
...
def transform_carryover(
    ...
    with numpyro.plate(
        name=f"{_AD_EFFECT_RETENTION_RATE}_plate", size=media_data.shape[1]
    ):
    ...
    with numpyro.plate(name=f"{_PEAK_EFFECT_DELAY}_plate", size=media_data.shape[1]):
...
def media_mix_model(
    ...
    with numpyro.plate(name=f"{_SIGMA}_plate", size=n_geos):
        sigma = numpyro.sample(
            name=_SIGMA, fn=custom_priors.get(_SIGMA, default_priors[_SIGMA])
        )
    ...
    with numpyro.plate(
        ...
        if media_data.ndim == 3:
            with numpyro.plate(name="geo_media_plate", size=n_geos, dim=-1):
    ...
    with numpyro.plate(name=f"{_GAMMA_SEASONALITY}_sin_cos_plate", size=2):
        with numpyro.plate(
            name=f"{_GAMMA_SEASONALITY}_plate", size=degrees_seasonality
        ):
    ...
    if weekday_seasonality:
        with numpyro.plate(name=f"{_WEEKDAY}_plate", size=7):
            weekday = numpyro.sample(
                name=_WEEKDAY, fn=custom_priors.get(_WEEKDAY, default_priors[_WEEKDAY])
            )
    ...
    if extra_features is not None:
        ...
        with numpyro.plate_stack(plate_prefixes, sizes=extra_features_plates_shape):
        ...
        extra_features_effect = jnp.einsum(
...
def multiplicative_media_mix_model(
    media_data: jnp.ndarray,
    target_data: jnp.ndarray,
    media_prior: jnp.ndarray,
    degrees_seasonality: int,
    ...
    """Media mix model (multiplicative version)."""
    ...
    # Intercept (transformed to positive scale)
    # Change: Use `jnp.exp` to ensure the intercept is strictly positive.
    with numpyro.plate(name=f"{_INTERCEPT}_plate", size=n_geos):
        intercept = jnp.exp(
            numpyro.sample(
                name=_INTERCEPT,
    ...
    # Trend component (transformed to positive scale)
    # Change: `coef_trend` is exponentiated for positivity, and the trend term is scaled multiplicatively.
    with numpyro.plate(name=f"{_COEF_TREND}_plate", size=n_geos):
        coef_trend = jnp.exp(
            numpyro.sample(
                name=_COEF_TREND,
    ...
    # Seasonality
    # Change: Seasonality is converted to a multiplicative factor by adding 1 (ensures positivity).
    with numpyro.plate(name=f"{_GAMMA_SEASONALITY}_sin_cos_plate", size=2):
        with numpyro.plate(
            name=f"{_GAMMA_SEASONALITY}_plate", size=degrees_seasonality
        ):
    ...
    # Extra features (multiplicative adjustment)
    # Change: Extra features contribute as multiplicative factors instead of additive terms.
    if extra_features is not None:
        plate_prefixes = ("extra_feature",)
        extra_features_einsum = "tf, f -> t"  # t = time, f = feature
        extra_features_plates_shape = (extra_features.shape[1],)
        ...
        extra_features_effect = jnp.exp(
            jnp.einsum(extra_features_einsum, extra_features, coef_extra_features)
        )
        prediction *= extra_features_effect  # Multiply by the extra features' effect

    # Weekday seasonality (multiplicative adjustment)
    # Change: Weekday seasonality is modeled as a multiplicative adjustment.
    if weekday_seasonality:
        with numpyro.plate(name=f"{_WEEKDAY}_plate", size=7):
            weekday = numpyro.sample(
                name=_WEEKDAY, fn=custom_priors.get(_WEEKDAY, default_priors[_WEEKDAY])
